#include "stdafx.h"
#include "SupoModel2.h"

Sim::SupoModel2::SupoModel2()
{
}
...
void Sim::SupoModel2::SetModelParameters()
{
	//-- Generic SYSTEM MODEL FOR A FISSILE SOLUTION SYSTEM (SUPO Version 2)
	//----------------------------------------------------------------------------------------------------
	//--
	...
	//--
	//--
	//-- Modifications incorporated in Version II
	//-- Thermal conductivity of water varied with coolent temperature
	//----- TKW=(0.001256*TCA+0.55588)*1e-06
	//----------- TCA=(TCA1+TCA2+TCA3+TCA4+TCA5+TCA6+TCA7+TCA8+TCA9+TCA10)/10
	//--------------- TCA1=(TC11+TC12)/2
	//--------------- TCA2=(TC21+TC22)/2
	//--------------- TCA3=(TC31+TC32)/2
	//--------------- TCA4=(TC41+TC42)/2
	...
	//--------------- TCA9=(TC91+TC92)/2
	//--------------- TCA10=(TC101+TC102)/2
	//-- Boiling point of water varied with pressure
	//----- TB=(1/(ln(1/PP)*2.045e-04+2.68e-03))-273.13
	//---------- PP: Plenum pressure in atm
	//-- Vary Plenum inlet pressure
	//----- P0=PP*1.0133e+05 | -- P0: Plenum inlet pressure in pa
	//-- Thermal conductivity of fuel varied with boundary layer temperature and uranium concentration
	//----- TKF=(-0.00017*FC+TKF0*1e+06)*1e-06
	//---------- TKF0=(0.001256*TLA+0.55588)*1e-06 | -- Thermal conductivity of pure water at BL temperature
	//-- Prandtl number of fuel varied with boundary layer temperature
	//----- Pr2=12.24769*(0.1*TLA)^(-0.8469)
	//-- alphaf: Isobaric compressibility of fuel (1/C)
	//----- alphaf=((0.00019*(0.1*TEMP)^3)-(0.00624*(0.1*TEMP)^2)+0.12101*(0.1*TEMP)-0.01837)*0.001
	//-- alphal: Isobaric compressibility of boundary layer (1/C)
	//----- alphal=((0.00019*(0.1*TLA)^3)-(0.00624*(0.1*TLA)^2)+0.12101*(0.1*TLA)-0.01837)*0.001
	//-- Expansion coefficient of boundary layer varied with temperature
	//----- bta=((0.00019*(0.1*TLA)^3)-(0.00624*(0.1*TLA)^2)+0.12101*(0.1*TLA)-0.01837)*0.001
	//-- Kinematic viscosity varied with boundary later temperature
	//----- kinvis=(1.7297*(0.1*TLA)^(-0.78195))*1e-06
	//-- Specific heat of fuel varied with fuel temperature
	//----- CVF=(0.0019*(0.1*TEMP)^2-0.0236*0.1*TEMP+4.2432)*1e-03
	//-- Specific heat of boundary layer varied with temperature
	//----- CVL=(0.0019*(0.1*TLA)^2-0.0236*0.1*TLA+4.2432)*1e-03
	//-- Thermal diffusivity of fuel varied with fuel density
	//----- THDIFF=TKF/(rho*CVL)
	//---------- rho=-0.24321*(0.1*TEMP)^2-2.11272*(0.1*TEMP)+1003.83908 | -- rho: density of fuel
	//-- Prandtl number of coolent varied with coolent temperature
	//----- Pr=12.24769*(0.1*TCA)^(-0.8469)
	//-- Bubble transit time varied by pressure and core height and referenced to SUPO
	//----- TAU=(HCORE/0.2303)*0.1818*tauf
	//---------- tauf=(3*PP/(4*PI))^(2/3)/(8*kinvis*161101)/holdup | -- tauf: Factor of change to transit time with pressure
	//---------- holdup=1 if no impediment to gas flow; else holdup=0.5707; SUPO and KEWB A-2 for example
	//-- Provide specific steam bubble transit time
	//----- TAUS=0.119*HCORE*PP^0.63522*tauf | -- TAUS: Steam bubble transit time in sec
	//-- Specific volume of gas varied with pressure
	//----- svg=1.72843*PP^(-0.95283) | -- (m^3/kg)
	//-- Specific heat of coolent varied with temperature
	//----- CPC=(0.0019*(0.1*TCA)^2-0.0236*0.1*TCA+4.2432)*1e-03
	//-- Gas threshold concentrations set by temperature, pressure, and uranium concentration
	//----- A1=-47.8948
	//----- A2=65.0368
	//----- A3=20.1709
	//----- B1=-0.0822
	...
	//----- B3=0.0079
	//----- Ts=(TEMP+273.15)/100
	//----- XTf=A1+A2*(1/Ts)+A3*ln(Ts)+(FC/1000)*(B1+B2*(Ts)+B3*(Ts)^2)
	//----- XTf1=exp(XTf)
	//----- XT=XTf1*(PP/7.10e+04) + swtch(T0*2-TEMP)
	//----- XTO=XT*(4.26e+04/7.10e+04)
	//----------------------------------------------------------------------------------------------------
	//-- Input Section
	//----------------------------------------------------------------------------------------------------
	//-- General Physical Parameters - Constants
	m_sdRG  =  8.31446;			// -- RG: Gas constant (m^3*Pa/K/mol)
	m_sdHM  =  (SIM_DATA)(2)/1000;			// -- HM: H2 molar mass (kg)
	m_sdOM  =  (SIM_DATA)(32)/1000;			// -- OM: O2 molar mass (kg)
	m_sdNM  =  (SIM_DATA)(28)/1000;			// -- NM: N2 molar mass (kg)
	m_sdPI  =  3.14159;
	m_sdUM  =  235.0;			// -- UM: Molar mass of U-235 (g)
	m_sdAN  =  6.022e+23;			// -- AN: Avogadro's Number
	m_sdkB  =  1.38e-23;			// -- kB: Boltzman constant (J/K)
	m_sdnmass  =  1.66e-27;			// -- nmass: Neutron mass (kg)
	...
	if (m_bIC) m_sdV0  =  0.011627;			// -- V0: Total initial core volume in m^3
	if (m_bIC) m_sdHCORE  =  0.21695;			// -- HCORE: Core height in m
	if (m_bIC) m_sdCSA  =  0.3648;			// -- CSA: Cooling surface area in m^2
	if (m_bIC) m_sdVP0  =  0.0026;			// -- VP0: Initial plenum volume in m^3
	if (m_bIC) m_sdCCR  =  0.0024;			// -- CCR: Cooling tube radius in m
	if (m_bIC) m_sdAREAHT  =  0.3648;			// -- AREAHT: Total cooling surface area in m^2
	if (m_bIC) m_sdMW  =  0.2;			// -- MW=mass of WALL (kg)
	if (m_bIC) m_sdCPW  =  0.5e-03;			// -- CPW=specific heat of WALL (MJ/C/kg)
	m_sdTNUM  =  1.0;			// -- TNUM=Number of cooling tubes
	m_sdholdup  =  0.5707;			// -- factor for configuration impeding gas flow; SUPO or KEWB for example
	//-- holdup=1 | -- factor for configuration with no impediment to gas flow
	//-- Reactivity Parameters
	if (m_bIC) m_sdALF  =  0.0344;			// -- ALF: Core averaged temperature coefficient of reactivity
	if (m_bIC) m_sdPHI  =  28.679;			// -- PHI: Void coefficient of reactivity
	...
	if (m_bIC) m_sdMNT  =  1.70e-04;			// -- MNT: Mean neutron generation time
	if (m_bIC) m_sdBETA  =  0.0085;			// -- BETA: Prompt neutron fraction
	//-- Operational Parameters
	if (m_bIC) m_sdT0  =  25.0;			// -- T0: Initial fuel temperature in deg C
	if (m_bIC) m_sdTC0  =  5.0;			// -- TC0: Coolent inlet temperature in deg C
	if (m_bIC) m_sdPP  =  0.8;			// -- PP: Plenum pressure in atm
}

void Sim::SupoModel2::InitializeModelVars()
{
	S2(m_sdVNI, 1.8333e-01);			// -- VNI: Flow rate of cover gas in m^/sec
	if (m_bIC) m_sdRRTE  =  0.01;			// -- RRTE: Reactivity insertion rate in $/sec
	if (m_bIC) m_sdFLAG  =  0.0;			// -- FLAG: Point-in-time-of reactivity insertion in sec
	if (m_bIC) m_sdRmax  =  1.90;			// -- Rmax: Total reactivity insertion in $
	m_sdWCR  =  0.2161;			// -- WCR: Coolant mass flow rate in kg/sec
	m_sdMCLUMP  =  0.0163;			// -- MCLUMP: Mass of coolant in each lump in kg
	if (m_bIC) m_sdFE  =  0.8879;			// -- FE: Fuel enrichment
	if (m_bIC) m_sdFC  =  84.2785;			// -- FC: Fuel Uranium concentration in g/l
	if (m_bIC) m_sdA  =  0.00;			// -- A: Initial Power ($)
	//-- Intrinsic Neutron Source Data
	if (m_bIC) m_sdQ0  =  1.0e+03;			// -- Q0=Initial intrinsic neutron source (#/s)
	m_sdEN0  =  1.0e-06;			// -- EN0=Inital power (MW)
	S2(m_sdEN, 1.0);S2(m_sdD1, 1.);S2(m_sdD2, 1.);S2(m_sdD3, 1.);S2(m_sdD4, 1.);S2(m_sdD5, 1.);S2(m_sdD6, 1.);
	//-- Simulation Control
	m_sdTMAX  =  500.0;			// -- TMAwX: Maximum time of the simulation in seconds
	//-- NN: Sampling points
	m_sdNN  =  40001;
	m_sdscale  =  20;			// -- scale: +/- limits of display variables
	//-- Display parameters
	...
	//-- AC=V0/HCORE | -- AC=Cross sectional area of bfz (m^2)
	//-- BL=1.00/1000 | -- BL=Boundary layer thickness (m)
	//-- AL=CSA/HCORE*BL | -- AL=Cross sectional area of blz (m^2)
	//-- VLAYER=AL*HCORE | -- VLAYER=Volume of blz (m^3)
	//-- EM=V0*1100.00/10 | -- EM=Total mass of fuel in bfz divided by # of fuel regions (kg)
	//-- ELM=VLAYER*1100.00/20 | -- ELM=Total mass of fuel in blz divided by # of fuel regions/2 (kg)
	//--
	m_sdLI  =  (SIM_DATA)(m_sdHCORE)/10;
	m_sdCTR  =  (SIM_DATA)(1)/100;
	//-- ALI=3.14159*((BL+0.18)*(BL+0.18)-0.18*0.18)
	//-- ALO=3.14159*(0.34*0.34-(0.34-BL)*(0.34-BL))
	//-- VL=VLAYER
	S2(m_sdWF, 0.00);
	m_sdTB  =  ((SIM_DATA)(1)/(log((SIM_DATA)(1)/m_sdPP)*2.045e-04+2.68e-03))-273.13;			// -- TB: Boiling point of water (C)
	m_sdP0  =  m_sdPP*1.0133e+05;			// -- P0: Plenum inlet pressure in pa
	m_sdHfg  =  (SIM_DATA)((-13.97367*m_sdPP+2178.67326))/1.0e+03;			// -- (MJ/kg)
	m_sdsvg  =  1.72843*pow(m_sdPP,(-0.95283));			// -- (m^3/kg)
	...
	m_sdTC1IN  =  m_sdTC0;			// -- TC1IN=Temperature of coolant entering core (C)
	...
	S2(m_sdTP2, m_sdT0);			// -- TP2=Inlet temperature to tube side (C)
	S2(m_sdTT1, m_sdT0);			// TT2=T0 | -- TT1,TT2=Tube side temperatures, first and second lumps (C)
	m_sdTS1IN  =  m_sdT0;			// -- TS1IN=Shell side inlet temperature (C)
	S2(m_sdTS1, m_sdT0);			// TS2=T0 | -- TS1,TS2=Shell side temperatures, first and second lumps (C)
	S2(m_sdTT, m_sdT0);			// TS=T0 | -- TT=Tube temperature (C) | TS=Shell temperature (C)
	//----- Initial values for plenum
	m_sdAIR  =  -1.0;			// -- AIR=Cover gas Flag
	S2(m_sdPN, m_sdP0*(0.79+0.21*swtch(m_sdAIR)));			// -- PN=Partial Pressure of N2 (Pa)
	S2(m_sdPO, m_sdP0*(0.21-0.21*swtch(m_sdAIR)));			// -- PO=Partial Pressure of O2 (Pa)
	S2(m_sdTP, m_sdT0+273);			// -- TP=Initial Temperature of plenum (K)
	S2(m_sdVP, m_sdVP0);			// -- VP=Plenum volume (m^3)
	m_sdPDOT  =  0.0;
	...
	m_sdRNI  =  (SIM_DATA)(V(m_sdPN, 0))/(SIM_DATA)(((SIM_DATA)(m_sdRG)/m_sdNM))/V(m_sdTP, 0);			// -- RNI=Density of N2 cover gas at plenum inlet (kg/m^3)
	m_sdROI  =  (SIM_DATA)(V(m_sdPO, 0))/(SIM_DATA)(((SIM_DATA)(m_sdRG)/m_sdOM))/V(m_sdTP, 0);			// -- ROI=Density of O2 cover gas at plenum inlet (kg/m^3)
	S2(m_sdVOGO, V(m_sdVNI, 0));			// -- ROI=Density of O2 cover gas at plenum inlet (kg/m^3)
	//--

	//----------------------------------------------------------------------------------------------------
}

void Sim::SupoModel2::ModelDerivatives(int iCol)
{
	...
	m_sdTCA  =  (SIM_DATA)((m_sdTCA1+m_sdTCA2+m_sdTCA3+m_sdTCA4+m_sdTCA5+m_sdTCA6+m_sdTCA7+m_sdTCA8+m_sdTCA9+m_sdTCA10))/10;
	...
	m_sdTLA  =  (SIM_DATA)((m_sdTLA1+m_sdTLA2+m_sdTLA3+m_sdTLA4+m_sdTLA5+m_sdTLA6+m_sdTLA7+m_sdTLA8+m_sdTLA9+m_sdTLA10))/10;
	m_sdPr  =  12.24769*pow((0.1*m_sdTCA),(-0.8469));			// -- Pr: Prandl number of coolant
	m_sdPr2  =  12.24769*pow((0.1*m_sdTLA),(-0.8469));			// -- Pr2: Prandl number of boundary layer
	m_sdCPC  =  (0.0019*pow((0.1*m_sdTCA),2)-0.0236*0.1*m_sdTCA+4.2432)*1e-03;			// -- CPC=Specific heat of coolent (MJ/C/kg)
	//-- Boundary layer geometry
	m_sdAC  =  (SIM_DATA)(m_sdV0)/m_sdHCORE;			// -- AC=Cross sectional area of bfz (m^2)
	m_sdrhol  =  -0.24321*pow((0.1*m_sdTLA),2)-2.11272*(0.1*m_sdTLA)+1003.83908;			// -- rhol: density of boundary layerl
	m_sddynvisl  =  1.78957*pow((0.1*m_sdTLA),(-0.82246));			// -- dynvisl: dynamic viscosity of boundary layer
	m_sdkinvis  =  (1.7297*pow((0.1*m_sdTEMP),(-0.78195)))*1e-06;			// -- kinvis: Kinematic viscosity of fuel
	//-- BL=1.0e-03 | -- BL=Boundary layer thickness (m)
	m_sdBL  =  0.382*((SIM_DATA)(m_sdHCORE)/10)*pow(((SIM_DATA)(m_sddynvisl)/(m_sdrhol*(V(m_sdWF, iCol)+0.0001)*((SIM_DATA)(m_sdHCORE)/10))),((SIM_DATA)(1)/5));
	//-- BLL=4.9*(kinvis*(HCORE/1)/(WF+0.0001))^0.5
	m_sdAL  =  (SIM_DATA)(m_sdCSA)/m_sdHCORE*m_sdBL;			// -- AL=Cross sectional area of blz (m^2)
	m_sdVLAYER  =  m_sdAL*m_sdHCORE;			// -- VLAYER=Volume of blz (m^3)
	m_sdEM  =  m_sdV0*(SIM_DATA)(1100.00)/10;			// -- EM=Total mass of fuel in bfz divided by # of fuel regions (kg)
	m_sdELM  =  m_sdVLAYER*(SIM_DATA)(1100.00)/20;			// -- ELM=Total mass of fuel in blz divided by # of fuel regions/2 (kg)
	m_sdVL  =  m_sdVLAYER;
	...
	m_sdCVF  =  (0.0019*pow((0.1*m_sdTEMP),2)-0.0236*0.1*m_sdTEMP+4.2432)*1e-03;
	m_sdCVL  =  (0.0019*pow((0.1*m_sdTLA),2)-0.0236*0.1*m_sdTLA+4.2432)*1e-03;
	m_sdAK  =  (SIM_DATA)(1)/(m_sdEM*m_sdCVF);
	m_sdAK2  =  (SIM_DATA)(1)/(m_sdELM*m_sdCVL);
	//--
	...
	//--
	m_sdR  =  m_sdA-m_sdRFBA-m_sdRFBV+m_sdRAMP;			// -- R=Reactivity of assembly ($)
	//--
	//------------------------------------------------------
	...
	//-------------------------------------------------------------------------------------------------------------
	m_sdmXS  =  582.0e-24;			//   -- mXS=U-235 microscopic fission XS at 293 K (cm^2)
	//--
	//-- Equation 5 of LA-UR-13-22033
	//--
	m_sdVav  =  pow((2*m_sdkB*(SIM_DATA)((273+m_sdTEMP))/m_sdnmass),0.5)*(SIM_DATA)(2)/pow(m_sdPI,0.5);			// -- Vav=Average speed of thermal neutrons (m/s)
	//--
	//-- Equation 6 of LA-UR-13-22033
	//--
	m_sdSav  =  m_sdmXS*(SIM_DATA)(pow(m_sdPI,0.5))/2*pow(((SIM_DATA)(293)/(273+m_sdTEMP)),0.5);			// -- Sav=Average microscopic fission X-section of thermal neutrons (cm^2)
	m_sdFND  =  (SIM_DATA)(m_sdFC)/1.0e+03*(SIM_DATA)(m_sdFE)/m_sdUM*m_sdAN;			// --  FND=U-235 number density (#/cm^3)
	//--
	//-- Equation 4 of LA-UR-13-22033
	//--
	m_sdPCF  =  1.0;			// -- PCF=Power Conversion Factor-3.1e+16 fissions/s=1 MW
	m_sdkw  =  (m_sdENP-m_sdEN0)*m_sdPCF*1.0e+03;			// -- kw=Total assembly power (kw)
	...
	m_sdRN2  =  (SIM_DATA)(m_sdWC)/m_sdTNUM*3*2*(SIM_DATA)(m_sdCCR)/(SIM_DATA)(m_sdACT)/0.001;
	...
	m_sdTHOT  =  m_sdTEMP;
	m_sdTCOLD  =  m_sdTWA;
	m_sdTKW  =  (0.001256*m_sdTCA+0.55588)*1e-06;			// -- TKW: Thermal conductivity of water
	m_sdTKF0  =  (0.001256*m_sdTLA+0.55588)*1e-06;			// -- TKF: Thermal conductivity of water at BL temperature
	m_sdTKF  =  (-0.00017*m_sdFC+m_sdTKF0*1e+06)*1e-06;			// -- TKF: Thermal conductivity of fuel
	//-- kinvis=(1.7297*(0.1*TEMP)^(-0.78195))*1e-06 | -- kinvis: Kinematic viscosity of fuel
	m_sdtauf  =  (SIM_DATA)(pow((3*(SIM_DATA)(m_sdPP)/(4*m_sdPI)),((SIM_DATA)(2)/3)))/(SIM_DATA)((8*m_sdkinvis*161101))/m_sdholdup;			// -- tauf: Factor of change from one atmosphere on transit time
	m_sdTAU  =  ((SIM_DATA)(m_sdHCORE)/0.2303)*0.1818*m_sdtauf;			// -- TAU: Bubble transit time in sec
	m_sdTAUS  =  0.119*m_sdHCORE*pow(m_sdPP,0.63522)*m_sdtauf;			// -- TAUS: Steam bubble transit time in sec
	m_sdTAU2  =  m_sdTAU;			// -- TAU,TAU2=Transit time of H2,O2 bubble in each fuel region i (s)
	//-- alphaf: Isobaric compressibility of fuel (1/C)
	m_sdalphaf  =  ((0.00019*pow((0.1*m_sdTEMP),3))-(0.00624*pow((0.1*m_sdTEMP),2))+0.12101*(0.1*m_sdTEMP)-0.01837)*0.001;
	//-- alphal: Isobaric compressibility of boundary layer (1/C)
	m_sdalphal  =  ((0.00019*pow((0.1*m_sdTLA),3))-(0.00624*pow((0.1*m_sdTLA),2))+0.12101*(0.1*m_sdTLA)-0.01837)*0.001;
	m_sdbta  =  ((0.00019*pow((0.1*m_sdTLA),3))-(0.00624*pow((0.1*m_sdTLA),2))+0.12101*(0.1*m_sdTLA)-0.01837)*0.001;			// -- bta: Expansion coefficient of fuel
	m_sdrho  =  -0.24321*pow((0.1*m_sdTEMP),2)-2.11272*(0.1*m_sdTEMP)+1003.83908;			// -- rho: density of fuel
	m_sdTHDIFF  =  (SIM_DATA)(m_sdTKF)/(m_sdrho*m_sdCVL);			// -- THDIFF: Thermal diffusivity of fuel
	//--
	//-- Setting threshold concentration
	m_sdA1  =  -47.8948;
	m_sdA2  =  65.0368;
	m_sdA3  =  20.1709;
	m_sdB1  =  -0.0822;
	m_sdB2  =  0.0496;
	m_sdB3  =  0.0079;
	m_sdTs  =  (SIM_DATA)((m_sdTEMP+273.15))/100;
	m_sdXTf  =  m_sdA1+m_sdA2*((SIM_DATA)(1)/m_sdTs)+m_sdA3*log(m_sdTs)+((SIM_DATA)(m_sdFC)/1000)*(m_sdB1+m_sdB2*(m_sdTs)+m_sdB3*pow((m_sdTs),2));
	m_sdXTf1  =  exp(m_sdXTf);
	m_sdXT  =  m_sdXTf1*((SIM_DATA)(m_sdPP)/7.10e+04)+swtch(m_sdT0*2-m_sdTEMP);
	m_sdXTO  =  m_sdXT*((SIM_DATA)(4.26e+04)/7.10e+04);
	m_sdDIA  =  2*m_sdCCR;
	...
	m_sdVFT1  =  (SIM_DATA)(m_sdV0)/10*m_sdalphaf*m_sdTEM1DOT;
	...
	m_sdVLT11  =  (SIM_DATA)(m_sdVL)/20*m_sdalphal*m_sdTL11DOT;
	...
	m_sdVLT12  =  (SIM_DATA)(m_sdVL)/20*m_sdalphal*m_sdTL12DOT;
	...
	m_sdVLT21  =  (SIM_DATA)(m_sdVL)/20*m_sdalphal*m_sdTL21DOT;
	...
	m_sdVLT22  =  (SIM_DATA)(m_sdVL)/20*m_sdalphal*m_sdTL22DOT;
	...
	m_sdVLT31  =  (SIM_DATA)(m_sdVL)/20*m_sdalphal*m_sdTL31DOT;
	...
	m_sdVLT32  =  (SIM_DATA)(m_sdVL)/20*m_sdalphal*m_sdTL32DOT;
	...
	m_sdVLT41  =  (SIM_DATA)(m_sdVL)/20*m_sdalphal*m_sdTL41DOT;
	...
	m_sdVLT42  =  (SIM_DATA)(m_sdVL)/20*m_sdalphal*m_sdTL42DOT;
	...
	m_sdVLT51  =  (SIM_DATA)(m_sdVL)/20*m_sdalphal*m_sdTL51DOT;
	...
	m_sdVLT52  =  (SIM_DATA)(m_sdVL)/20*m_sdalphal*m_sdTL52DOT;
	...
	m_sdVLT61  =  (SIM_DATA)(m_sdVL)/20*m_sdalphal*m_sdTL61DOT;
	...
	m_sdVLT62  =  (SIM_DATA)(m_sdVL)/20*m_sdalphal*m_sdTL62DOT;
	...
	m_sdVLT71  =  (SIM_DATA)(m_sdVL)/20*m_sdalphal*m_sdTL71DOT;
	...
	m_sdVLT72  =  (SIM_DATA)(m_sdVL)/20*m_sdalphal*m_sdTL72DOT;
	...
	m_sdVLT81  =  (SIM_DATA)(m_sdVL)/20*m_sdalphal*m_sdTL81DOT;
	...
	m_sdVLT82  =  (SIM_DATA)(m_sdVL)/20*m_sdalphal*m_sdTL82DOT;
	...
	m_sdVLT91  =  (SIM_DATA)(m_sdVL)/20*m_sdalphal*m_sdTL91DOT;
	...
	m_sdVLT92  =  (SIM_DATA)(m_sdVL)/20*m_sdalphal*m_sdTL92DOT;
	...
	m_sdVLT101  =  (SIM_DATA)(m_sdVL)/20*m_sdalphal*m_sdTL101DOT;
	...
	m_sdVLT102  =  (SIM_DATA)(m_sdVL)/20*m_sdalphal*m_sdTL102DOT;
	...
	m_sdVFT2  =  (SIM_DATA)(m_sdV0)/10*m_sdalphaf*m_sdTEM2DOT;
	...
	m_sdVFT3  =  (SIM_DATA)(m_sdV0)/10*m_sdalphaf*m_sdTEM3DOT;
	...
	m_sdVFT4  =  (SIM_DATA)(m_sdV0)/10*m_sdalphaf*m_sdTEM4DOT;
	...
	m_sdVFT5  =  (SIM_DATA)(m_sdV0)/10*m_sdalphaf*m_sdTEM5DOT;
	...
	m_sdVFT6  =  (SIM_DATA)(m_sdV0)/10*m_sdalphaf*m_sdTEM6DOT;
	...
	m_sdVFT7  =  (SIM_DATA)(m_sdV0)/10*m_sdalphaf*m_sdTEM7DOT;
	...
	m_sdVFT8  =  (SIM_DATA)(m_sdV0)/10*m_sdalphaf*m_sdTEM8DOT;
	...
	m_sdVFT9  =  (SIM_DATA)(m_sdV0)/10*m_sdalphaf*m_sdTEM9DOT;
	...
	m_sdVFT10  =  (SIM_DATA)(m_sdV0)/10*m_sdalphaf*m_sdTEM10DT;
	...
	//-- Steam model
	//---------------------------------------------------------------------------------------------------------------
	m_sdP  =  V(m_sdPN, iCol)+V(m_sdPH, iCol)+V(m_sdPO, iCol);			//   -- Plenum/Fuel Pressure (Pa)
	//--
	//-- 10
	//--
	...
	//---------------------------------------------------------------------------------------------------------------
	//-- Nodes 1-10, 1=bottom
	m_sdMC11  =  m_sdMCLUMP;			// -- MC11=Mass of Node 1 lump 1 (kg)
	m_sdMC12  =  m_sdMCLUMP;			// -- MC12=Mass of Node 1 lump 2 (kg)
	m_sdFR11  =  0.5;			// -- FR11=Fraction of Region 1 power going into lump 1
	m_sdFR12  =  0.5;			// -- FR12=Fraction of Region 1 power going into lump2
	//-- EQUATION 50 of LA-UR-13-22033
	m_sdTAUC11  =  (SIM_DATA)(m_sdMC11)/m_sdWC;			// -- TAUC11=Transit time Node 1 lump 1 (s)
	m_sdTAUC12  =  (SIM_DATA)(m_sdMC12)/m_sdWC;			// -- TAUC12=Transit time Node 1 lump 2 (s)
	...
	m_sdMP2  =  20.0;			// -- MP2=Mass of coolant in pipe (kg)
	//-- EQUATION 51 of LA-UR-13-22033
	m_sdTAUP2  =  (SIM_DATA)(m_sdMP2)/m_sdWC;			// -- TAUP2=Transit time of piping (s)
	...
	//-- Tube side
	//---------------------------------------------------------------------------------------------------------------
	m_sdMT1  =  m_sdMC11*4*10;			// -- MT1=Mass of lump (kg)
	m_sdTAUT1  =  (SIM_DATA)(m_sdMT1)/m_sdWC;			// -- TAUT1=Transit time of lump (s)
	m_sdGAM1  =  m_sdHT2*m_sdAREAHT*4;			// -- GAM1=Overall heat transfer coeff. - Tube side to tube (MW/C)
	//-- *EQUATION 52
	D(m_sdTT1, (SIM_DATA)(2)/m_sdTAUT1*(V(m_sdTP2, iCol)-V(m_sdTT1, iCol))-(SIM_DATA)(m_sdGAM1)/(SIM_DATA)(m_sdMT1)/m_sdCPC*(V(m_sdTT1, iCol)-V(m_sdTT, iCol)), iCol);			// -- TT1=Temperature of first lump (C)
	//--  EQUATION 53 of LA-UR-13-22033
	D(m_sdTT2, (SIM_DATA)(2)/m_sdTAUT1*(V(m_sdTT1, iCol)-V(m_sdTT2, iCol))-(SIM_DATA)(m_sdGAM1)/(SIM_DATA)(m_sdMT1)/m_sdCPC*(V(m_sdTT1, iCol)-V(m_sdTT, iCol)), iCol);			// -- TT2=Temperature of second lump (C)
	//---------------------------------------------------------------------------------------------------------------
	//-- Piping from HE to Core
	//---------------------------------------------------------------------------------------------------------------
	m_sdMP1  =  20.0;			// -- Mass of coolant in pipe (kg)
	m_sdTAUP1  =  (SIM_DATA)(m_sdMP1)/m_sdWC;			// -- TAUP1=Transit time in piping (s)
	//---------------------------------------------------------------------------------------------------------------
	//-- Shell side
	//---------------------------------------------------------------------------------------------------------------
	m_sdWC2  =  3.0;			// -- WC2=Mass flow rate (kg/s)
	m_sdMS1  =  2*m_sdMT1;			// -- MS1=Mass of lump (kg)
	m_sdTAUS1  =  (SIM_DATA)(m_sdMS1)/m_sdWC2;			// -- TAUS1=Transit time of lump (s)
	m_sdGAM2  =  m_sdGAM1;			// -- GAM2=Overall heat transfer Coeff. - Tube to shell side (MW/C)
	m_sdGAM3  =  m_sdGAM1;			// -- GAM3=Overall heat transfer Coeff. - Shell side to shell (MW/C)
	...
	//-- Tube
	//---------------------------------------------------------------------------------------------------------------
	m_sdMT  =  10.0;			// -- MT=mass of tube (kg)
	m_sdCPT  =  0.5e-03;			// -- CPT=specific heat of tube (MJ/C/kg)
	...
	//-- Shell
	//---------------------------------------------------------------------------------------------------------------
	m_sdMS  =  20.0;			// -- MS=Mass of shell (kg)
	m_sdCPS  =  0.5e-03;			// -- CPS=Specific heat of shell (MJ/C/kg)
	...
	m_sdGH2  =  (SIM_DATA)(m_sdMH2)/0.5;			// -- (kg/MJ)  H2
	m_sdGO2  =  (SIM_DATA)(m_sdMO2)/0.5;			// -- (kg/MJ) O2
	...
	//-- Plenum Volume
	//-- Water make-up model
	m_sdWMUIN  =  0.0;			// -- WMUIN=water mass flow in (kg/s)
	//-- EQUATION 24 of LA-UR-13-22033
	m_sdWMUDOT  =  m_sdWMUIN-m_sdROH2A*(SIM_DATA)(V(m_sdVGH10, iCol))/m_sdTAU-m_sdROO2A*(SIM_DATA)(V(m_sdVGO10, iCol))/m_sdTAU2;			// -- WMUDOT=time rate of change of makeup water mass
	D(m_sdWMU, m_sdWMUDOT, iCol);			//  --  WMU=Mass of makeup water (kg)
	m_sdROW  =  1000.0;			// -- ROW=Density of makeup water (kg/m^3)
	//-- EQUATION 25 of LA-UR-13-22033
	m_sdVFDOT  =  (SIM_DATA)(m_sdWMUDOT)/m_sdROW;			// -- VFDOT=time rate of change of makeup water volume
	//---------------------------------------------------------------------------------------------------------------
	//-- Fuel Expansion
	//---------------------------------------------------------------------------------------------------------------
	...
	m_sdVPDOT  =  -(m_sdVHDT1+m_sdVHDT2+m_sdVODT1+m_sdVODT2+m_sdVFT);			// -- VPDOT=time rate of change of plenum volume
	D(m_sdVP, m_sdVPDOT, iCol);			// -- VP=Volume of plenum (m^3)
	//--  Check Plenum Volume
	m_sdVCTOT  =  m_sdVFUEL+m_sdVHTOT+m_sdVOTOT;			// --  VCTOT=Total Core Volume (m^3)
	//---------------------------------------------------------------------------------------------------------------
	...
	m_sdCPS  =  1900.0;			// -- J/kg/C  specific heat of steam
	...
	m_sdRHY  =  (SIM_DATA)(V(m_sdMHY, iCol))/V(m_sdVP, iCol);			// -- RHY=H2 density in plenum (kg/m^3)
	m_sdROX  =  (SIM_DATA)(V(m_sdMOX, iCol))/V(m_sdVP, iCol);			// -- ROX=O2 density in plenum (kg/m^3)
	m_sdRN  =  (SIM_DATA)(V(m_sdMN, iCol))/V(m_sdVP, iCol);			// -- RN=N2 density in plenum (kg/m^3)
	...
	D(m_sdMHY, m_sdMHDOT, iCol);			// -- MHY=H2 mass in plenum (kg)
	...
	D(m_sdPH, m_sdPHDOT, iCol);			// -- PH=Partial pressure of H2 in plenum (Pa)
	...
	D(m_sdMOX, m_sdMODOT, iCol);			// -- MOX=O2 mass in plenum (kg)
	//-- EQUATION 17 of LA-UR-13-22033
	...
	D(m_sdPO, m_sdPODOT, iCol);			// -- PO=Partial pressure of O2 in plenum (Pa)
	//-- Nitrogen
	//-- EQUATION 18 of LA-UR-22033
	m_sdMNDOT  =  m_sdRNI*V(m_sdVNI, iCol)-m_sdRN*V(m_sdVOGO, iCol);
	D(m_sdMN, m_sdMNDOT, iCol);			// -- MN=N2 mass in plenum (kg)
	...
	D(m_sdPN, m_sdPNDOT, iCol);			// -- PN=Partial pressure of N2 in plenum (Pa)
	m_sdPDOT  =  m_sdPHDOT+m_sdPODOT+m_sdPNDOT;			// -- PDOT=time rate of change of plenum pressure
	//-- Check PN
	//-- PN=RN*RG/NM*TP=MN/VP*RG/NM*TP
	//-- Gas Summaries
	m_sdVGS  =  V(m_sdVS1, iCol)+V(m_sdVS2, iCol)+V(m_sdVS3, iCol)+V(m_sdVS4, iCol)+V(m_sdVS5, iCol)+V(m_sdVS6, iCol)+V(m_sdVS7, iCol)+V(m_sdVS8, iCol)+V(m_sdVS9, iCol)+V(m_sdVS10, iCol);
	m_sdVGH  =  V(m_sdVGH1, iCol)+V(m_sdVGH2, iCol)+V(m_sdVGH3, iCol)+V(m_sdVGH4, iCol)+V(m_sdVGH5, iCol)+V(m_sdVGH6, iCol)+V(m_sdVGH7, iCol)+V(m_sdVGH8, iCol)+V(m_sdVGH9, iCol)+V(m_sdVGH10, iCol);
	m_sdVGO  =  V(m_sdVGO1, iCol)+V(m_sdVGO2, iCol)+V(m_sdVGO3, iCol)+V(m_sdVGO4, iCol)+V(m_sdVGO5, iCol)+V(m_sdVGO6, iCol)+V(m_sdVGO7, iCol)+V(m_sdVGO8, iCol)+V(m_sdVGO9, iCol)+V(m_sdVGO10, iCol);
	m_sdVG  =  m_sdVGS+m_sdVGH+m_sdVGO;
	//---------------------------------------------------------------------------------------------------------------
	//-- graph display
	//---------------------------------------------------------------------------------------------------------------
	D(m_sdE, m_sdPCF*(m_sdENP-m_sdEN0), iCol);
	m_sdENPMAX  =  m_sdENPMAX+(m_sdENP-m_sdENPMAX)*swtch(m_sdENP-m_sdENPMAX);
	m_sdkwmax  =  m_sdENPMAX*m_sdPCF*1.0e+03;
	D(m_sdKF, m_sdkw*3.3e+13, iCol);
	m_sdFWHM  =  (SIM_DATA)(V(m_sdKF, iCol))/(SIM_DATA)(m_sdkw)/3.3e+13;
